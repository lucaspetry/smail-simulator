...
function Dashboard() {
    ...
    function segColor(c) {
        return { Sucesso: "#41ab5d", Falha:"#d83838", Adiamento:"#e08214" }[c];
    }
    ...
    this.build = function(id, fData){
        freqData = fData;
        
        // compute total for each state.
        freqData.forEach(function(d) {
            d.total = d.freq.Sucesso + d.freq.Falha + d.freq.Adiamento;
        });
        ...
        tF = ['Sucesso', 'Falha', 'Adiamento'].map(function(d){ 
            return {type:d, freq: d3.sum(freqData.map(function(t){ return t.freq[d];}))}; 
        });    

        // calculate total frequency by state for all segment.
        sF = freqData.map(function(d){return [d.State,d.total];});
    ...
    this.update = function(fData) {
        freqData = fData;
        freqData.forEach(function(d){d.total=d.freq.Sucesso+d.freq.Falha+d.freq.Adiamento;});
        tF = ['Sucesso', 'Falha', 'Adiamento'].map(function(d){ 
            return {type:d, freq: d3.sum(freqData.map(function(t){ return t.freq[d];}))}; 
        });
